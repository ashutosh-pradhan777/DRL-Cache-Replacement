    ...
    def _run(self):
        ...
        for lba, write, _ in self.reader.read():
            ...
            if op == CacheOp.INSERT:
                ...
                if lba not in self.seen_blocks:
                    self.seen_blocks.add(lba)
                    self.compulsory += 1
                elif evicted is not None:
                    if hasattr(alg, 'lfu') and evicted in getattr(alg, 'lfu', {}):
                        self.conflict += 1
                    else:
                        self.capacity += 1
